...
var APP = {
  ...
  HEADERS: {
    // As colunas de senha são armazenadas como hash + salt (nunca a senha em texto).
    USUARIOS: ["ID", "EMAIL", "NOME", "DATA_CADASTRO", "SENHA_HASH", "SALT"],
...
function handleRequest_(e, method, parsedBody) {
  try {
    ...
    if (route === "/register" && method === "POST") {
      return successResponse_("Cadastro realizado com sucesso.", handleRegister_(body));
    }
...
/**
 * Faz o login do usuário e devolve sessão autenticada.
 */
function handleLogin_(body) {
  var email = normalizeEmail_(body.email);
  var senha = sanitizeText_(body.senha, 120);
  ...
  var computedHash = hashPassword_(senha, String(userRecord.SALT || ""));
  if (computedHash !== String(userRecord.SENHA_HASH || "")) {
...
/**
 * Faz o cadastro do usuário e devolve sessão autenticada.
 */
function handleRegister_(body) {
  var nome = sanitizeText_(body.nome, 80);
  var email = normalizeEmail_(body.email);
  var senha = sanitizeText_(body.senha, 120);
  ...
  var senhaHash = hashPassword_(senha, salt);
  var now = new Date().toISOString();

  var sheet = getSheet_(APP.SHEETS.USUARIOS);
  sheet.appendRow([userId, email, nome, now, senhaHash, salt]);
  ...
  var settings = getOrCreateUserSettings_(userId);
  var token = issueSessionToken_(user);
...
/**
 * Normaliza e-mail para garantir unicidade.
 */
function normalizeEmail_(value) {
  return sanitizeText_(value, 200).toLowerCase();
}
...
/**
 * Gera um salt aleatório (base64 web-safe).
 */
...
/**
 * Hash de senha usando SHA-256(salt + ":" + senha).
 * Observação: Apps Script não oferece bcrypt/scrypt nativamente.
 * Para produção em cenários críticos, considere usar um provedor de auth dedicado.
 */
function hashPassword_(senha, salt) {
  var material = String(salt || "") + ":" + String(senha || "");
...
function findUserByEmail_(email) {
  ...
  for (var i = 0; i < records.length; i += 1) {
    if (normalizeEmail_(records[i].EMAIL) === normalized) {
      return records[i];
    }
  }
  return null;
...
// Login Google removido por enquanto. Mantemos apenas login normal (e-mail/senha).
...
// Propriedade ALLOWED_GOOGLE_CLIENT_IDS não é usada nesta versão (sem login Google).
